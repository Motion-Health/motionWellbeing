...
export const AnalyticsTable = () => {
  ...
  useGetAllAccounts();
  const { data: activities, refetch: refetchActivities } = useListActivitiesWithMetrics();
  const { data: completedActivities, refetch: refetchCompletedActivities } = useCompletedActivities();
...
useEffect(() => {
    ...
    <div className="announcements">
      <div>
        ...
        {tabValue === 0 && (
          ...
          <tbody>
          {tabValue === 0 && loadedAccounts?.allAccounts
  ...
  .map((account) => (
        ...
        {tabValue === 1 && (
          ...
          <tbody>
          {tabValue === 1 && displayActivities && displayActivities.map((activity) => (
...
{tabValue === 2 && (
          ...
          {tabValue === 2 && completedActivities && completedActivities.map((activity) => {
              ...
              <tr key={activity.logId}>
                <td>
                  {displayActivities?.find(
                    (activity) => activity.activityId === activity.activityId
                  )?.activityName}

                </td>
                <td>
                  {loadedAccounts?.allAccounts.find(
                    (account) => account.accountId === activity.accountId
                  )?.serviceProviderName}
                </td>
